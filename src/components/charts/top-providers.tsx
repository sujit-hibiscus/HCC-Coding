...
export function TopProviders({ data, isLoading, isExpanded }: TopProvidersProps) {
  const [activeKeys, setActiveKeys] = useState<string[]>(["V24", "V28"]);
      ...
      >
        <ChartContainer
          config={{
            V24: {
              label: "V24",
              color: colorConfig.V24,
            },
            V28: {
              label: "V28",
              color: colorConfig.V28,
            },
          }}
          ...
          <ResponsiveContainer width={chartWidth || "100%"} height="100%">
            <BarChart data={displayData} margin={{ top: 10, right: 10, left: 0, bottom: 20 }} barGap={4}>
              ...
              <motion.g variants={itemVariants}>
                {activeKeys.includes("V24") && (
                  <Bar
                    dataKey="V24"
                    stackId="a"
                    fill={colorConfig.V24}
                    radius={[0, 0, 0, 0]}
              ...
              <motion.g variants={itemVariants}>
                {activeKeys.includes("V28") && (
                  <Bar
                    dataKey="V28"
                    stackId="a"
                    fill={colorConfig.V28}
                    radius={[4, 4, 0, 0]}
                    animationDuration={1000}
                    animationEasing="ease-in-out"
                  />
